...
假設 repo 為 `Training`、reviewer 為 `Gaduo`，你（committer）是 `PG`：

    TCU-MI/Training -----------------------------------> PG/Training
           ^                                                      |
           |                                                      |
           +------------------ Gaduo/Training <----------------+


PG 的 repo 是由 `TCU-MI/Training` fork 出來（等同於 Github 幫你作 `git clone`），
task 完成後發 pull request 給 reviewer，
reviewer 如果 accept，就會整合回 `TCU-MI/Training`。
...
1. 在 `WtfTask` 上頭作業
1. 完成後發 pull request 給 reviewer（`Gaduo/Training`），
